...
with tab1:
    ...
    with col_input:
        ...
        st.caption("Adjust the parameters below to estimate loyalty and cancellation risk under both AI models.")
    ...
    with st.expander("🛠️ Advanced Profile Settings (Demographics & Services Defaults)", expanded=False):
        st.caption("These defaults complete the model inputs. Tweak them to simulate specific customer details.")
    ...
    with col_result:
        ...
        st.caption("Compare real-time risk predictions generated by AI Model A and AI Model B.")
        ...
        st.markdown(get_verdict_card("AI Model A (Trend Analyzer)", st.session_state.lr_prob), unsafe_allow_html=True)
        
        # Display KNN Card
        st.markdown(get_verdict_card("AI Model B (Similar-Customer Matcher)", st.session_state.knn_prob), unsafe_allow_html=True)
